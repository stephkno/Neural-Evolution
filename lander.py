...
def preprocess(x):
    ...
    #x = numpy.sum(x/255.0, axis=2)/3.0
    #x = (x > 0.5) * 1.0
    #x = skimage.measure.block_reduce(x, (8,8), numpy.max)
    #cv2.imshow("img", x)
    #cv2.waitKey(1)
    return numpy.array(x/16.0).flatten()
...
initial_hidden_node_size = 20
initial_genome_size = 50
    ...
    for i in range(int(num_species-len(new_agents))):
        ...
        if numpy.random.random() > 0.5:
            if numpy.random.random() > 0.5:
                ...
                baby.mutate_gene(mutation_curve(i))
        new_agents.append(baby)

    return new_agents

#get first generation of agents
agents = [get_new_agent(inputs, outputs) for _ in range(num_species)]
